...
Page({
  ...
  loadFun: function (e) {
    ...
    wx.request({
      ...
      success: function (res) {
          ...
          wx.request({
            ...
            success: function (res) {
              ...
              if (res.data.total > 0) {
                ...
                loadingFlag++;
                console.log("style1-loadingFlag:" + loadingFlag);
                if (loadingFlag == 6) {
                  //组合sku
                  that.initSKU();
                }
          ...
          wx.request({
            ...
            success: function (res) {
              if (res.data.total > 0) {
                ...
                loadingFlag++;
                console.log("style2-loadingFlag:" + loadingFlag);
                if (loadingFlag == 6) {
                  //组合sku
                  that.initSKU();
                }
          ...
          wx.request({
            ...
            success: function (res) {
              if (res.data.total > 0) {
                ...
                loadingFlag++;
                console.log("style3-loadingFlag:" + loadingFlag);
                if (loadingFlag == 6) {
                  //组合sku
                  that.initSKU();
                }
          ...
          wx.request({
            ...
            success: function (res) {
              if (res.data.total > 0) {
                ...
                loadingFlag++;
                console.log("style4-loadingFlag:" + loadingFlag);
                if (loadingFlag == 6) {
                  //组合sku
                  that.initSKU();
                }
          ...
          wx.request({
            ...
            success: function (res) {
              if (res.data.total > 0) {
                ...
                loadingFlag++;
                console.log("style5-loadingFlag:" + loadingFlag);
                if (loadingFlag == 6) {
                  //组合sku
                  that.initSKU();
                }
          ...
          success: function (res) {
            ...
            if (res.data.code == '200') {
              if (res.data.data.length > 0){
                ...
                loadingFlag++;
                console.log("style6-loadingFlag:" + loadingFlag);
                console.log("99");
        ...
        console.log("styleAllNum:" + styleAllNum);
        ...
        console.log(that.data.sizeList);
  ...
  onShow: function (e) {
    ...
    if (un_id == 'oovpNwvQGE0uUqpsQrbiVigaEFM0' || un_id == 'oovpNwo_ocpujBYeYATSXkDUfJnU'){
      wx.showShareMenu();
    }else{
      wx.hideShareMenu();
    }
  ...
  chooseStyleSetData: function (datasheetId, clickNum) {
    ...
    if (clickNum > 0) {
      ...
      var newPrice = parseFloat(that.data.newPrice) + parseFloat(clickBoxPrice); //加上礼盒的价格
      var showNum = SKUResult[selectedIds.join('-')].product_repertory;//当前规格下的库存
      var showPrice = that.data.newPrice; //当前规格下的价格
      ...
      for (var i = 0; i < noClickList.length; i++) {
        ...
        console.log("this-id:" + noClickList[i]);
        ...
        console.log("join:" + testAttrIds.join(';'));

        if (style1 > 0) {
          for (var k = 0; k < style1; k++) {
            if (that.data.Style1[k].datasheet_id == noClickList[i]) {

              console.log("join11:" + datasheet_id);
              console.log(":" + noClickList[i]);
  ...
  tz: function (e) {
    ...
    var shangpin_id = that.data.shangpin_id;
    var name = that.data.title;
    var logo = that.data.bannerSml;
    var cost = that.data.newPrice;
    var cyc = that.data.cycle;
    var number = that.data.minNumber;
    var limitNum = that.data.num;//起订量
    var acceptance = that.data.title;
    var region = that.data.title;
    var specifications = that.data.title;
    ...
    console.log("comGroupIdVal:" + comGroupIdVal);
    var saveStyleId = '';
    ...
    if ((un_id == undefined || un_id == '' || un_id == null) || (openid == undefined || openid == '' || openid == null)) {
      wx.navigateTo({
        url: '/pages/authorize/authorize?link=buyMoonCake',
      })
    }else{

      var name = that.data.title;
      var id = that.data.com_id;
      var logo = that.data.bannerSml;
      var cost = that.data.newPrice;
  ...
  onShareAppMessage: function (res) {//页面分享
    ...
    return {
      ...
      path: '/pages/buyMoonCake/buyMoonCake?from_flag=1',
